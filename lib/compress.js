...
const { convertImagesToDataUrls } = require('./image-data-url');
...
const zipIt = async (epubContents) => {
  try {
    ...
    for(let i=0, max=epubContents.contents.length; i<max; i++) {
      const epubContent = epubContents.contents[i];
      const imageReplacements = await convertImagesToDataUrls(epubContent.html);
      const xhtml = xmlgen.createContentXHTML(epubContent, i, max, imageReplacements);
