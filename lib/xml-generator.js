...
const createContentXHTML = (content, idx, contentLength, imageReplacements) => {
...
const createOPFXMLString = (options) => {
  ...
  const { author, publisher, cover, title, contents } = options;
  ...
  if (cover) {
    ...
    const coverItemImage = xmlbuilder.begin()
      .ele('item', {
        id: 'cover_image',
        href: cover.split('/').pop(),
        'media-type': getMime(cover)
      });
...
const createCoverXHtml = (coverLocation) => {
  const html = xmlbuilder.begin()
    ...
    .ele('title', {}, 'Cover').up()
    .up()
    .ele('body', { style: 'height: 100%'})
    .ele('div', { style: 'display: table; height: 100%; margin: 0 auto' })
    .ele('div', { style: 'display: table-cell; vertical-align: middle;' })
